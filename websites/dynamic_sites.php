<?php $title = "dynamic Sites" ?>
<?php $description = "Hurry Up Print dynamic websites, build dynamic site, custom dynamic site" ?>
<?php require('../head.php') ?>
<?php require('../header.php'); ?>
<div class="container">
<!-- BREADCRUMBS -->
<nav class="yellow darken-1 m2a">
  <div class="nav-wrapper ms2">
    <div class="col s12">
      <a href="/index.php" class="breadcrumb grey-text text-lighten-1"><i class="material-icons">home</i></a>
      <a href="/websites.php" class="breadcrumb grey-text text-lighten-1">Websites</a>
      <a href="/websites/dynamic_sites.php" class="breadcrumb black-text">Dynamic Sites</a>
    </div>
  </div>
</nav>

<section class="flyers__section">
<h1>Dynamic Sites</h1>
  <div class="divider"></div>
 <?php require('../under_construction.php') ?>
</section>

</div>

<?php require('../footer.php'); ?>